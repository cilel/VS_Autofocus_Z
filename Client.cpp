...
/** -------Global variables for controlling ------**/
    ...
    double Sg_sum; // current cost function (sum of square of image gradient)
    double vgd_p; // cost function in last iteration

    double lambda=2e-3; // gain for controlling x,y
...
vpColVector semPosCont(Mat curImage, Mat desImage, int iter)
{
    // 1. Conver to Visp
    ...
    else
    {
        ...
        else if(pjModel==parallelZ)
        {
           ...
           //cout << "Sg_sum and Sgd_sum:" <<  Sg_sum << " , " << Sgd_sum << endl;
            ...
            //vgd = sign*fabs(alpha*(Sg_sum-Sgd_sum)/Jg[0]); // for positioning
            //cout << "vgd=" << vgd << endl;
